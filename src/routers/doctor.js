...
router.post('/doctors', async (req, res) => {
    try {
        ...
        if (!name || !email || !password || !profession) {
            return res.status(400).send({ error: 'Name, email, password and profession are required' });
        }

        // Implement logic to create a new doctor in the database
        const newDoctor = await Doctor.create({ name, email, password, profession });
...
router.get('/doctors/:id', async (req, res) => {
    try {
        ...
        // Implement logic to fetch a doctor from the database by ID
